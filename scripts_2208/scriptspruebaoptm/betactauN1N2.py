...
def main(parameters):
    ...
    file_out = destiny + f'full_{base_out}.hepmc'
    ...
    for sentence in hepmc:
        ...
        if len(line) > 0:
            ...
            #estamos guardando el varcode en una variable
                outg = int(line[1])
                info = *[float(x) for x in line[3:6]], ctau, int(line[8]) # x,y,z, ctau, number of outgoing
    ...
    for sentence in hepmc:
        ...
        if len(line) > 0:
            if line[0] == 'E':
                ...
                #viene_del_neutrino = outg in selection
                ...
                if(outg in selection and pid == 9900014):
                    x, y, z, ctauN16 = [d_scaler*ix for ix in holder['v'][outg][0:4]]
                else:
                    ctauN16 = 0.0
