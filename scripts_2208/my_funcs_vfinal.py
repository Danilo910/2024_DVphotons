            ...
            for index_ph, row_ph in event_ph.iterrows():
                ...
                for index_d, row_d in event_surr.iterrows():
                    ...
                    if same and (index_ph == index_d):
                        dr = dR*1000
                    if dr < dR:
                        #print("Entramos al caso jets cerca de un muon")
                        contador_jets_cerca_muones += 1 
                        cone += row_d[obs]
                        if(row_ph.Constituents == 13):
                            contador_jets_cerca_muones_jets_un_muon += 1
                        # print("obs")
                        # print(obs)
                        # print("Row_d")
                        # print(row_d[obs])
...
def convert_single_element_arrays(constituents):
    # If the length of the list is 1, return the integer inside it, else return the original list
    if len(constituents) == 1:
        return int(constituents[0])
    return constituents