...
def cone_isolation(track_pt, tower_et, muon_pt, track_pdg, delta_r_tracks, delta_r_towers, delta_r_max= 0.2, pt_min = 0.5):
    ...
    if delta_r_tracks is not None:
        ...
        #track_pdg_mask considera solo los casos dentro del arreglo que tienen muon o antimuon
        # a los que tienen muon los caracteriza con 1 y los que no tienen con None
        ...
        #tendremos una matriz donde solo se considera los tower que son muones
        pdg_within_mask = np.array(within_cone_tracks_int) * track_pdg_mask
        ...
        if np.any(np.all(delta_r_filtered == 0, axis=1)):
            ...
            zero_rows = np.where(np.all(delta_r_filtered == 0, axis=1))[0]
            print(f"Error: Row(s) {zero_rows} contain only zeros. Aborting operation.")
        else:
            # Replace zeros with np.inf temporarily to avoid selecting them as minimum
            mod_deltar_filtered = np.where(delta_r_filtered == 0, np.inf, delta_r_filtered)

            # Find the index of the minimum value in each row
            min_deltar = np.argmin(mod_deltar_filtered, axis=1)
