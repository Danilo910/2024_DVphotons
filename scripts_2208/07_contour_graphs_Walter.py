...
for delta in deltas[1:]:
    ...
    for s_up, channels in sigmas_up.items():
        ...
        for name in names[:]:
            ...
            #It captures the number after M and before the next underscore _
            ...
            #con esto extraemos el tipo de proceso que tenemos si es TTH, WH o ZH
            ...
            cofre = {key: np.asarray(matrix) for key, matrix in info.items()}

            #recordar que channels es una lista, pero solo tiene 1 o 1, +2, no confundir con que tiene los dos canales
            #simultaneamente
            ...
            #En la primera iteracion, solo nos dara un elemento info, pues solo analiza el canal 1.
            #en el caso 1 , +2 que es la segunda iteracion alli si sumara el del canal 1 con el +2

        # Grouping the list of values by same mass and alpha
        
        #Se esta creando un diccionario de la siguiente forma:
        # {(m,alpha): [("WH", 2.4), ("ZH", 1.2), ("TTH", 0.5)]}
        #asi repetidas veces variando el alpha
        
        print('values saliendo del loop')
        print(values)
        ...
        #el primer proc[0] nos da que tipo de evento es, TTH, WH o ZH

        #A cada dupla interna ("channel", number) de # {(m,alpha): [("WH", 2.4), ("ZH", 1.2), ("TTH", 0.5)]}
        #se le multiplica el k_factors[proc[0]] del canal correspondiente.
        ...
        #se suma sobre todos los tipos de evento, aun quedara la estructura de (m,alpha) con alpha variando,
        #pero ahora ("WH", 2.4), ("ZH", 1.2), ("TTH", 0.5) se colapsa y seria 2.4 + 1.2 + 0.5
        points = {key: sum(val) for key, val in points.items()}
        
        print("points despues de sumar")
        print(points)
        sys.exit("Salimos")
