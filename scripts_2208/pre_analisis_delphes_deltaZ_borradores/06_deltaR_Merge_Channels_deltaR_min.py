...
def merge_and_plot(deltaR_min, mode, alpha):
    
    # Directory where txt files are stored
    #origin = f"/Collider/scripts_2208/data/clean/deltaR_{mode}"
    origin = f"/Collider/scripts_2208/data/clean/{deltaR_min}/"
    ...
    destiny = f"./data/{deltaR_min}/Merge/{mode}/{alpha}/"
...
for deltaR_min in deltaR_mins
    ...
    for mode in modes:
        ...
        for alpha in [4, 5, 6]:
            print(f"alpha: {alpha}")
            merge_and_plot(deltaR_min, mode, alpha)