...
void Isolation::Init()
{
  ...
  fActivateMuonIso = GetBool("ActivateMuonIso", false);
...
void Isolation::Process()
{
  ...
  // Determine which ratio to use for the final isolation cut
  if (fActivateMuonIso)
  {
    // Correct sum without pile-up contamination, instead of that, we use the formula 
    // isolation_ratio = isolation_ratio_tracks + 0.4 * isolation_ratio_towers
    sum_Muon_Iso = sumChargedNoPU + 0.4*sumNeutralEt

    ratio_Muon_Iso = sumChargedNoPU / candidateMomentum.Pt() + 0.4*sumNeutralEt / candidateMomentum.Et();

    // Store results in the candidate object
    // mandamos a cero la variable
    candidate->IsolationVar = 0;
    // esto seria lo correcto candidate->Isolation_muon_Ratio = ratio_Muon_Iso, pero para evitar fallos hacemos
  ...
  else
  {
    ...
    // Default logic: skip candidates that fail sum-based isolation criteria
