...
def main(variables):
    ...
    # ------------------------------------------------
    # Save BEFORE overlapping
    # ------------------------------------------------
    output_dir = "/Collider/2023_LLHN_CONCYTEC/scripts_2208/crisvswd/"
    os.makedirs(output_dir, exist_ok=True)

    # Define the full output paths (before overlapping)
    output_path_muons = os.path.join(output_dir, f"muons_filtered_Wanilo_before_overlapping_{type}.pkl")
    output_path_electrons = os.path.join(output_dir, f"electrons_filtered_Wanilo_before_overlapping_{type}.pkl")
    output_path_jets = os.path.join(output_dir, f"jets_filtered_Wanilo_before_overlapping_{type}.pkl")
    output_path_photons = os.path.join(output_dir, f"photons_filtered_Wanilo_before_overlapping_{type}.pkl")

    # Save muons, electrons, jets, and photons as pickle files
    muons = leptons[leptons.pdg == 13]
    electrons = leptons[leptons.pdg == 11]

    muons.to_pickle(output_path_muons)
    print(f"Muons DataFrame saved to {output_path_muons}")

    electrons.to_pickle(output_path_electrons)
    print(f"Electrons DataFrame saved to {output_path_electrons}")

    jets.to_pickle(output_path_jets)
    print(f"Jets DataFrame saved to {output_path_jets}")

    photons.to_pickle(output_path_photons)
    print(f"Photons DataFrame saved to {output_path_photons}")
    ...
    output_path_muons2 = os.path.join(output_dir, f"muons_filtered_Wanilo_after_overlapping_{type}.pkl")
    output_path_electrons2 = os.path.join(output_dir, f"electrons_filtered_Wanilo_after_overlapping_{type}.pkl")
    output_path_jets2 = os.path.join(output_dir, f"jets_filtered_Wanilo_after_overlapping_{type}.pkl")
    output_path_photons2 = os.path.join(output_dir, f"photons_filtered_Wanilo_after_overlapping_{type}.pkl")
